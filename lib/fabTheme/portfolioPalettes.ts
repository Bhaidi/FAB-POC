...
/**
 * Dark operational block — matches {@link GlassSurface} `card` (liquid glass: blur + gradient fill + rim).
 * Prefer wrapping with `GlassSurface variant="card"`; this sx stays for any legacy `sx={...}` usage.
 */
export const portfolioOperationalSurfaceSxDark = {
  position: "relative" as const,
  overflow: "hidden" as const,
  background: glassTokens.fill.card,
  border: `1px solid ${glassTokens.border.default}`,
  boxShadow: glassTokens.shadowStack.card,
